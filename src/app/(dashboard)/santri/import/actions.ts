...
export async function bulkImportSantri(data: ImportedSantri[]): Promise<ImportResult[]> {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        try {
            ...
            // Validate jenjang
            if (row.jenjang !== "SMP" && row.jenjang !== "SMA") {
                results.push({
                    success: false,
                    row: rowNum,
                    nis: row.nis,
                    nama: row.nama,
                    error: "Jenjang harus SMP atau SMA",
                });
                continue;
            }
            ...
            try {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const { data: authData } = await (adminClient as any).auth.admin.createUser({
                    email: autoEmail,
                    password: password,
                    email_confirm: true,
                });

                // Create profile with ortu role if auth succeeded
                ...
                // Ignore auth errors for now (e.g., duplicate email)
            }
